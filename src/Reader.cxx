...
namespace scheme
{
   ...
   {
      List* list = Memory::nil;
      List* last = Memory::nil;
	 ...
	 switch ( ch )
	 {
	    ...
	    default:
	    {
		  ...
		  return list;
	       ...
	       {
		  last = list = Memory::cons( n, Memory::nil );
	       }
	       else
	       {
		  List* newCell = Memory::cons( n, Memory::nil );
		  last->setcdr( newCell );
		  last = newCell;
	       }
	    }
   ...
   {
      Node* n = readExpr( inport );         // <expr>
      n = Memory::cons( n, Memory::nil );   // (<expr> . ())  == (<expr>)
      n = Memory::cons( flavor, n );        // (<flavor> . (<expr> . ())) == (<flavor> <expr>)
      return n;
  }
